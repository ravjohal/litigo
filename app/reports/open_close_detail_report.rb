class OpenCloseDetailReport < Dossier::Report
...
# ABOVE NOTATION TRANSLATES TO EXACT SQL:
 ...
 # WHERE "notes"."firm_id" = $1 AND "contacts"."user_account_id" = 23
  ...
  def sql
    Case.where("cases.firm_id = :firm_id 
    	AND cast(closing_date as date) between :start_date and :end_date"
    	).group("cases.id").select("case_number, name, subtype, cast(created_at AS date), closing_date"
    	).to_sql
