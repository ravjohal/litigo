...
function CasesChart(application) {
    ...
    this.tip = d3.tip().attr("class", "d3-tip").html(function(d) {
        return "<table cellpadding='7em'>" +
            "<tr><td><strong>Amount: </strong></td><td>" + (d["Is_na"] ? "NA" : ("$" + self.format(d["Total_amount"]))) + "</td></tr>" +
            "<tr><td><strong>Judge: </strong></td><td>" + d["Judge"] + "</td></tr>" +
            "<tr><td><strong>County: </strong></td><td>" + d["County"] + "</td></tr>" +
        "</table>";
    });
