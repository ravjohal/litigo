class ImportToDocxService
  ...
  def generate_docx
    tmp_file = Htmltoword::Document.create_with_content(Htmltoword::Document.default_xslt_template, @name, @html)
    ...
    new_tmp_file = Tempfile.new(@name)
    ...
    doc.paragraphs.each_with_index do |p, index|
      ...
      if html_row_class =~ /columns/
        ...
        if html_row_class =~ /two_columns/
          ...
          p.node.children.first.add_previous_sibling wppr if p.node.children.first
