class LeadsDatatable
  ...
  def data
    leads.map do |lead|
      [
          ...
          content_tag(:div, '', class: "status-circle #{lead.status}", data: {tip: "#{Lead::STATUS[lead.status.to_sym] if lead.status}"}).html_safe,
      ]
    end
  end
  ...
  def sort_column
    columns = %w[created_at attorney injured_party_name estimated_value status]
